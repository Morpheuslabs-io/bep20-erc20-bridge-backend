...
const BSCPayer = function () {
    ...
    this.MAX_GASPRICE = new BigNumber(Util.toWei(this.web3, Config.BSC_SWAP_GASPRICE_LIMIT, 'gwei'));
}

inherits(BSCPayer, BaseComponent);

BSCPayer.prototype.start = function () {
    setInterval(this.startPayer.bind(this), this.PROCESS_INTERVAL * 60 * 1000);
    this.log('Payer is started');
}

BSCPayer.prototype.sendFillTx = async function (events, gasPrice, walletAddress, walletPrivateKey) {
    try {
        this.log(`Processing ${events.length} ETH2BSC events via wallet ${walletAddress}...`);

        const payload = this.swapContract.methods.fillETH2BSCSwap(
        ...
        const gas = Util.getBscGas(events.length);
        ...
        tx = { ...tx, transactionHash: signed.transactionHash, network: Constant.NETWORK.BSC }
        TxManager.add(tx);
...
BSCPayer.prototype.startPayer = async function () {
    try {
        if (this.isProcessing) return;
        this.isProcessing = true

        // check pending events
        const unprocessedEvents = await DB.getUnprocessedEvents(Constant.NETWORK.ETH);
        ...
        let wallets = WalletManager.getWallets(Constant.NETWORK.BSC);
        wallets = wallets.filter(wl => !wl.isInUsed);

        if (wallets.length == 0) {
            this.log(`There is no hotwallet available for ${Constant.NETWORK.BSC} network`);
            this.isProcessing = false;
            return;
        }
        ...
        for (let wallet of wallets) {
            ...
            WalletManager.setIsInUsed(wallet.address, Constant.NETWORK.BSC, true);
